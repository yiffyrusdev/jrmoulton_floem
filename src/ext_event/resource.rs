...
/// # Memoization
///
/// The resource automatically memoizes results based on the dependency value. If the dependency
/// hasn't changed since the last fetch, no new async operation is started. Dependencies are
/// compared using `PartialEq`.
///
...
impl<T> Resource<T> {
    ...
    ///
    /// This is a convenience wrapper around [`new_full`](Self::new_full) that uses the dependency
    /// value itself as the comparison key. Use this when your dependency type is cheap to clone
    /// and implements `PartialEq + Hash`.
    ///
    /// # Parameters
    ///
    /// * `source` - A function that returns the current dependency value(s). This function
    ///   is called reactively, and when its return value changes (compared with `PartialEq`),
    ...
    /// value with the previous one using `PartialEq`. Only when the dependency actually changes
    ...
    where
        ...
        Dep: Hash + Clone + Send + 'static,
    {
        Self::new_full(source, |dep| dep.clone(), fetcher)
    }
        ...
        K: Hash + 'static,
    {
        ...
        cx.create_effect(move |last| {
            ...
            let should_fetch = match last {
                None => true,
                Some((last_hash, last_refetch_count)) => {
            ...
            if !should_fetch {
                return (current_hash, refetch_count);
            }
    ...
    /// - You're dealing with time-sensitive data that should always be refreshed
    ///
    /// # Concurrency
    ///
    /// Each dependency change spawns a new tokio task. If multiple tasks complete while the main
    ...
    /// - Clears the current data (sets to `None`)
